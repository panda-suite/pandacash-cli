...
async function seedAccounts() {
  ...
  keyPairs.forEach(async (keyPair) => {
    try {
      await exec(`docker exec bch-regtest bitcoin-cli -regtest -rpcuser=regtest -rpcpassword=regtest importaddress ${keyPair.address}`)
  ...
  await exec('docker exec bch-regtest bitcoin-cli -regtest -rpcuser=regtest -rpcpassword=regtest generate 500');
}
